...
> ScribeFlow 把“从视频到笔记”变成一张可编辑的加工流：素材在左，节点连线决定加工方式，右侧属性调每一步，运行后直接在画布上看到每步产出。
...
1. **视频观点笔记**（默认模板）：`B站链接` → `转写` → `AI 校对` → `观点提炼` → `输出(Markdown)`。
2. **技术教程拆解**：`B站链接` → `转写` → `技术文案提炼` → `输出`；分支加一条 `信息溯源` 并行输出。
3. **多提示词对照**：`文稿` → 并行 `观点提炼` / `自定义提示词 A` / `自定义提示词 B` → `合并节点` → `输出`，一眼比较三个版本。
4. **已有文稿加工**：`文本节点`（粘贴文稿）→ `校对` → `提炼` → `输出`，跳过下载与 ASR。
...
| [n8n](https://n8n.io) | **节点面板 + 画布 + 属性检查器**三段布局；工作流作为工程保存；节点状态可视化；运行日志可追溯 | 触发/集成节点的复杂度；fair-code 不能抄代码，只能复刻交互 |
...
| 7 | 节点底部输出缩略（`12 个观点块 · 1.2k 字`）→ 点击看详情 | n8n run data | 行为复刻，详情进右侧检查器/弹窗 |
...
| 13 | 输出结果面板（Tab 切换多输出） | Flowise/Dify | 行为参考；做成右侧检查器 Tab + 运行详情页 |
...
| `/project/:id` | 画布编辑器：左节点库 / 中画布 / 右检查器 / 底运行控制台 |
...
**左：节点库（200px，可折叠）**
- 分组：来源（B站链接/本地文件/文本）、转写、AI 加工（校对/观点提炼/技术文案/信息溯源/自定义提示词）、组织与输出（合并/输出）。
- 拖入画布创建；顶部搜索框；最近使用置顶（照搬 n8n 节点创建器行为）。

**中：画布（Vue Flow，直接套用引擎能力）**
- 浅灰点阵底、平移/缩放、框选、小地图；节点/连线/端口行为见 3.3 照搬清单。
- 节点卡片：`[线框图标] 节点名` + 状态点（灰=未运行/蓝=运行中/绿=完成/红=失败）+ 端口；运行后显示产物摘要行。
- 端口类型校验：`音频→转写`、`文稿→AI/合并`、`笔记块→合并/输出`。

**右：检查器（280–320px，可折叠）**
- 未选中：工程信息 + 运行默认值（默认 AI 模型/ASR 引擎、并发）。
- 选中节点：
  - B 站链接：URL 输入与解析预览（封面/分 P 勾选）＋按钮**「从我的 B 站选择」**（打开快捷选择弹窗）
  - 本地文件：选择/拖入、大小、格式提示
  - 文本：粘贴区
  - 转写：ASR 引擎覆盖
  - AI 节点：提示词块下拉 + 「覆盖编辑」展开、模型覆盖、输出命名
  - 合并：输入排序、合并标题
  - 输出：Markdown 预览、复制、下载
- Tab：`属性` / `运行记录`（本工程运行列表）。

**底：运行控制台（28–36px 状态条，可展开 240px 面板）**
- 收起：`运行 #12 · 3/5 节点完成 · 转写中 42%`；展开：节点任务行（阶段中文文案、进度条、耗时、取消/重试）。

### 5.3 B 站来源快捷选择（来源步骤的一部分，非独立收藏模块）

- 入口：B 站来源节点检查器「从我的 B 站选择」；未登录时先弹 400px 登录弹窗（仅扫码）。
...
**前端画布（直接套用，不自研）**
- 引擎：`@vue-flow/core` 全套（节点/边/Handle/MiniMap/Controls/useVueFlow），自研仅限：节点卡片样式、节点库、检查器、来源选择器、状态徽章。
...
| M1 工程与画布 | 工程列表、Vue Flow 画布接入、节点库、连线/撤销/自动布局、检查器、自动保存、导入导出 | 3.3 照搬清单第 1–11 项全部按 n8n 行为打勾 |
