...
- 应用外壳：浅色、纸灰画布、白表面、发丝边界、墨色导航（样式方向见 `apps/web/src/styles/tokens.css` 与 [AGENTS.md](../../AGENTS.md) 硬规则第 2 条）。
...
| D2 | UI 组件路线 | ~~Tailwind CSS 4 + Radix Vue 自建~~ **已被取代**：采用 Element Plus 2.14 作唯一通用底座，见 [ui-library-replacement-research.md](./ui-library-replacement-research.md) |
...
- 内部规范：[AGENTS.md](../../AGENTS.md)（项目约定与设计令牌规则）
