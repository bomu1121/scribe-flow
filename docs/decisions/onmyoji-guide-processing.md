...
- 仓库内已有调研：[ai-tagging-design.md](./ai-tagging-design.md)（受控标签优于自由生成）、[workflow-module-expansion-research.md](../research/workflow-module-expansion-research.md)（内容加工流不要直接抄 iPaaS 功能）；多步配方 + 断言门见 `packages/shared/src/recipe.ts`。
