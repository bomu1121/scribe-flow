...
> 状态：M0–M6 全部完成。ScribeFlow 里程碑开发收官。
...
## 15. 素材挑选：多素材链路只加工其中几段（2026-09-10）

- 需求（用户提出）：一张来源卡承载多个视频/分P 时，希望能指定「只让其中几个走后续流程」，而不是必须全部走；并希望入口隐蔽一点（放高级设置）。
- 先摸清现状再动手：引擎本来就是「一个输入一份结果」——`resolveInputs()` 把上游产出铺成 `items[]`（转写 `engine.ts` 逐个音频转写、AI 分支逐个输入跑配方、文本工具逐项映射），结果页的分段大纲正是从这套 per-item 数据还原的。**瓶颈不在能力，而在两处「压平」**：`process.merge` / `process.output` 直接把 `items.map(...).join()` 拼成一整篇，`flow.if` 也只在整篇上判断。
- 三项决策（用户拍板）：① 只做「挑段」不做分组多支路；② 用**勾选真实素材**指定，而不是填序号；③ 未选中的素材**连同其下游一起跳过**。
- 契约（`packages/shared`）：
  - `NodeBase.data.pick: Record<来源节点 id, 素材段标识[]>`——**键缺失 = 该来源全选**（旧工程零迁移），空数组 = 明确排除；全选时界面会删掉该键，保持工程数据干净。
  - `NodeOutput.itemKey`：产物自带素材身份，缺失表示不可筛选的整体产物（合并文档），一律通过。
  - `RunNodeInput.itemKey / excluded`：结果页据此说明「共几段、本次加工几段」。
  - 新增 `src/segment.ts` 纯函数：`segmentKey`（`bvid:<bvid>:<分P>` / `file:<fileId>` / `node:<来源节点>`，**只用素材自身稳定信息，不掺数组下标**，避免上游改选区后静默错位）、`passesPick`、`collectSegmentOptions`（收集上游可挑素材）、`stalePickKeys`、`isSourceOutputNeeded`。引擎与界面共用同一套判定，不各写一份。
- 引擎（`apps/server`）：
  - `resolveInputs()` 在铺完 `items` 后按 `pick` 过滤，**未选中的素材根本不进节点**——「下游一并跳过」是这个设计的自然结果，没有额外的传播逻辑；过滤后一段不剩则拒绝执行并提示可操作文案（不是静默跑出空笔记）。
  - `itemKey` 打点与传递：来源卡按 `bvid+分P` 打点，转写/AI/文本工具按「一个输入一份产出」把身份传给下游（`pairItemKeys` 下标优先 + 唯一匹配兜底），合并/输出保持「整体产物无身份」。
  - **来源卡自己也会跳**：`isSourceOutputNeeded` 判断「所有下游都不要这段」就不下载——避免「没选它却仍然下完再丢」；节点摘要写明「2 个音轨已就绪（另跳过 1 个未选中素材）」。判定只在消费者节点**明确配了挑选**时才可能为假，因此未配置挑选的旧链路行为完全不变。
  - 落库：未选中素材也写一行 `run_node_inputs`（`excluded=1`）作说明用，但不参与「本节点消费了什么」的展示（现有分段逻辑按正文非空过滤，天然不受影响）；数据库补 `item_key` / `excluded` 两列，幂等迁移。
  - 顺手修掉一个真缺陷：`resolveInputs` 在 `try` 之外，取输入阶段抛错时节点会**永远卡在「运行中」**；现在该阶段失败也会落一条失败状态并广播 `node.error`。
- 交互（`apps/web`）：新增 `PickFields.vue` 挂进节点高级设置，**只有上游存在两张及以上来源卡时才渲染**——单素材链路里它和「高级设置」入口都不会多出来，不需要靠藏。行内复选框 + 每组全选/全不选 + `已选 2 / 3 段`；上游换过选区导致挑选失效时点名提醒。
- 结果页：输出文档抬头在挑选生效时显示「这份文档只加工了选中的素材：共 3 段，另有 1 段未选中、本次未加工」，从输出节点沿链路回溯汇总所有排除行——避免把「按挑选产出」误读成「笔记丢了内容」；节点摘要同样带「处理 2/3 段」。
- 自检：`pnpm -r typecheck / test / build`、`pnpm lint` 全绿；server 81 例（新增 `engine.pick.test.ts` 6 例：缺省全选、部分选、全排除、身份穿透、界面侧依据、段标识）。
- 真实浏览器验收（离线链路：三份文稿 → 合并 → 输出，未改动用户数据）：
  - 画布合并节点高级设置内出现「素材挑选 / 只加工选中的素材 / 3 / 3 段」+ 3 个复选框；取消勾选第二份 → `2 / 3 段`，工程即刻持久化为 `{"n_t2":[]}`。
  - 从界面点「运行」：`n_merge [done] 32 字 · 已按挑选执行：处理 2/3 段（跳过 1 段）`；输入行 `node:n_t2 excluded=true`；日志「素材挑选：已跳过 1 段未选中素材（node:n_t2）」；产物由 45 字降为 32 字且不含乙的正文。
  - 结果页抬头出现上述说明行（视觉复核：位置在标题与统计行之间，颜色/字号层级正常，无溢出）。
- 验收中发现的两处既有行为（**非本功能引入**，已确认不影响本功能正确性）：① 未解析的 B 站来源卡在画布上不渲染其连线，而引擎与结果页侧栏读的是工程图（因此选择器改为读画布自己的 `nodesRef/edgesRef` 快照，而非 Vue Flow store）；② 结果页「链路输入」侧栏只列出原始素材层，中间节点（如合并）不在其中（原有分层逻辑所致）。二者如需一并收敛，建议单独开一轮。
- 待办：`pnpm smoke:ui` 全量回归（本轮未跑，属既有回归项）；「按内容自动挑段」（把 `flow.if` 升级为逐段判断）与「分组多支路」是明确的下一步候选，本轮未做。

## 16. 「素材挑选」独立节点 `flow.pick`（2026-09-10）

- 需求修正（用户指出 §15 理解偏了）：要的不是「藏在消费节点高级设置里的勾选框」，而是**一个能拖进画布的独立模块**——接在上游模块之后，自己去识别上一环给出的那一堆内容，然后勾选放行其中几段。用户同步否掉了自己先提的「放高级设置」，理由是独立节点更好。
- 结论：用户是对的，且更贴房内范式（`flow.if` / `process.text` 就是这类中间件）。§15 的引擎侧实现（`pick` 过滤 + 段标识）**完全复用**，本轮只补「节点形态」这一层。
- 新增节点 `flow.pick`（`packages/shared`）：`NODE_PORTS` 单进单出、多类型透传（`transcript | noteBlock | noteDoc`），数据复用 `NodeBase.data.pick` 与第 15 节同一份；`emptyNodeData`、zod schema、节点库「文本与逻辑」组（漏斗图标）一并补齐。
- 引擎分支（`engine.ts`）：到达该节点时挑选已在 `resolveInputs` 生效，本节点只把选中的**原样放行**并保留素材身份，摘要写「放行 2/3 段」。
- **关键接缝**：段标识（`segmentKey`）是「设计时勾选」与「运行时实际集合」之间唯一的对应关系。为此修掉一个会破坏它的隐患——`resolveInputs` 原本无条件按「产出方 + 位置」重新派生段标识，会把挑选节点透传的原始身份换成 `pos:<挑选节点>:0`，导致**在挑选节点之后的下游再挑一次时一段都选不中**。现在产出方自带身份时优先沿用。
- 界面：`PickCard.vue`（挑选节点卡片，直接列出识别到的素材 + 勾选 + 每组全选/全不选 + `2 / 3 段`）；「上游可挑素材」抽成 `composables/useSegmentPick.ts`，挑选节点卡片与消费节点高级设置**共用同一份计算**，避免两处口径走偏。
- 自检：`pnpm -r typecheck / test / build`、`pnpm lint` 全绿；server 86 例（新增 `engine.picknode.test.ts` 5 例：默认全放行、勾选后只有选中的流到下游、**挑选节点之后还能再挑一次**（身份不被换成位置键的定向用例）、全不勾则拒绝、连接时识别上游集合）。
- 真实浏览器验收（三份文稿 → 素材挑选 → 合并 → 输出，未改动用户数据）：节点库可见新节点；拖入后卡片列出「第一份 / 第二份 / 第三份」与 `3 / 3 段`；取消勾选第二份 → `2 / 3 段` 并持久化 `{"n_2":[]}`；运行后 `n_pick [done] 放行 2/3 段 · 已按挑选执行：处理 2/3 段（跳过 1 段）`，下游合并节点只收到 `node:n_1`/`node:n_3`（身份是原始素材的，不是挑选节点的位置键），产物 32 字且不含第二份正文。
- 保留：消费节点高级设置里的同款选择器**没有删**——它与独立节点写的是同一份 `data.pick`，互为两种用法（就近配置 vs 图上显式分流）。若日后认为重复，删任一处都不影响另一处。
- 待办：`pnpm smoke:ui` 全量回归；「挑选节点上游是 AI 集合（元素无稳定标识）时如何挑选」目前按「无身份即不可筛选、一律放行」处理，若要支持需给 AI 集合定义稳定序号标识——属后续项。

### 16.1 用户反馈修正（同日）

- **「左侧栏没有素材挑选」——根因不是代码，是开发服务器**：运行了两小时的 Vite 实例（15:02 启动，期间改了 20+ 文件）HMR 模块图已脏，导致**整个节点面板空白**（只剩「工程/运行库/节点」三个 tab），控制台抛 `Cannot read properties of null (reading 'shapeFlag')`。硬刷新无效，清 `node_modules/.vite` 也无效（脏的是进程内存）。**重启一个干净的 vite 实例后，节点面板 5 组 17 项全部正常，「素材挑选」就在「文本与逻辑」组（文本工具与条件分支之间）**。教训：长时间运行的 dev server 在大量改动后应重启，不要用它验收 UI；后续 UI 验收改为「重启 dev server → 再截图」。
- **「卡片没有描述、内容区留白」——三个真实缺陷，已修**：
  1. **描述被省略号截断**：`flow.pick` 是唯一把说明放在**头部**的卡片，而头部 `.sf-node-desc` 是 `nowrap + ellipsis`（空间有限），我原先那句 19 字必然被截。改为 9 字「只放行其中几段素材」，实测 253/253px 不截断。
  2. **「全不选 / 全选」按钮看不见**：分组标题是 `flex: 0 1 auto`，被长标题挤压到宽度归零。卡片宽度由 264 提到 280，并给组名加 `flex: 1 1 auto; min-width: 0`，三个按钮实测均可见（19/13/19px）。
  3. **底部多出一行空白**：卡片高度跟随内容回弹，列表最后一项与结果栏之间只剩正常间距（视觉复核确认已无突兀空白）。
  4. 顺带把重复入口去掉：`flow.pick` 自身的挑选就在正文里，高级设置里的同款选择器与入口对该类型不再出现（消费节点保留）。
- **顺带修掉一个既有缺陷**：`.sf-node-desc--block` 这个 class **在整个工程里没有对应 CSS 规则**（一直是个空类），因此练一练卡片（`DrillCard`）与攻略节点正文里的说明文字同样被单行截断。已补规则（`white-space: normal`），实测练一练卡片说明换行正常、无溢出、无破版。
- 自检：`pnpm -r typecheck / test / build`、`pnpm lint` 全绿（server 86 · web 14 · shared 65）；视觉复核确认描述完整、按钮齐全、无多余空白。

### 16.2 关键修正：接在「中间模块」后识别不到素材（同日，用户第二次纠正）

- 用户指出的场景（原话）：「一个校对模块处理了八个输入，他应该就会出现对这八个模块允许进行选择的操作」——并追问「是你没设计，还是因为工程是老数据」。
- **结论：是我没设计。** 现场取证（`prj_701e09b9` 网络安全工程，真实 8 视频运行 `run_a0bc492d`）：
  - `n_src[source.bili items=8] → n_asr（8 个音频·111917 字）→ n_refine（8 个输入·44437 字）→ n_mtvpdum6001（flow.pick）`；
  - `n_refine` 在 `run_node_inputs` 里就是 **8 行、每行一份独立结果**——引擎侧一直是分的；
  - 但我的 `collectSegmentOptions` **只从来源卡收集素材**，走到 `n_refine` 这种中间模块时既不是来源卡、也不再往上追 → 返回空 → 卡片没有任何可勾选项。
  - 第一次做「连接时识别」时我只想到「多选来源卡」这一种形态，**漏了「中间模块的集合」**，而后者正是用户最初描述的形态。
- 顺带发现同一条链上第二处错误：过滤条件写成 `cards.length <= 1 → []`（按**来源卡张数**判断），于是「一张卡承载 8 个素材」也被判成「没得挑」。改为按**素材总数**判断（`options.length > 1`）。
- 修法：识别改为「沿入边一路往上游追」——
  - 来源卡：按卡内素材逐个列出（一张多选卡 = N 个素材）；
  - 中间模块：**穿过去**继续追（转写 / 校对 / AI 加工 / 攻略 / 练一练 / 文本工具 / 挑选），因此「校对模块处理了 8 个输入」能被识别成 8 个可选项；
  - 压平型模块（合并 / 输出 / 章节 / 导图 / 条件分支）：身份到此为止——继续往上找到的是它各入边上的素材（「只让第一份进合并」讲得通），但那些素材与它压平后的产物没有一一对应关系，**想按段分流必须发生在压平之前**（即用「素材挑选」节点拦在中间）。
- **一处被自己测试抓到的真回归**：我一度把 web 的 `PER_INPUT_RESULT_TYPES` 换成 shared 的 `PER_INPUT_NODE_TYPES`，结果文本工具的分段退回成未加工的原文（`run-segments.test.ts` 直接失败）。二者语义不同——前者是「结果页怎么解析该节点自己的产出」，后者是「素材身份能否穿过该节点」——已回退并写明理由，避免以后又合并。
- 共享侧新增 `segment.test.ts` 6 例（多选卡 8 项、穿过转写+校对仍识别 8 项、单素材不展示、多张卡汇入、压平节点之后仍可挑入边素材、失效键点名）。
- 真实工程验收：`prj_701e09b9` 的挑选节点现在列出 **8 / 8 段**（含每集标题与时长），可逐集勾选；顺带修掉重复显示（B 站卡的 `part` 常与 `title` 同值，重复会重复显示两遍）与单组时多余的分组标题行。
- 自检：`pnpm -r typecheck / test / build`、`pnpm lint` 全绿（shared 71 · server 86 · web 14）。

### 16.3 事故与防护：工程图被空画布覆盖（同日）

- **事故**：在上述修改过程中，`prj_701e09b9`（网络安全-负基础基础篇，8 视频工程）的工程图被写成空图（`nodes=0 edges=0`），用户后加的 3 个节点（AI 加工 / 素材挑选 / 知识巩固）全部丢失。数据库 WAL 写入时间 23:56:21 与我在该工程开着编辑器期间改组件（HMR 替换组件树）的时间吻合。
- **原因**：`ProjectEditorView` 已有一道「防误清空护栏」（注释记录 2026-09-10 曾发生过一次），但它依赖前端时序（`lastSavedNodeCount` 与当前 `projectId` 匹配），在热更新换树/渲染异常的路径下没能兜住。**我不该在一个正开着编辑器的工程上做大批量改动。**
- **恢复**：用该工程一次成功运行留下的图快照（`runs.graph_json`，`run_a0bc492d`）恢复主干 5 节点 4 边；另 3 个节点按我此前读取到的 id/类型/连线重建。**节点、连线、标签、来源卡的 8 个素材选择全部还原；位置是我按原布局估的**（快照里没有这 3 个节点），可能需要微调。
- **防护（已实施，不依赖前端时序）**：`PUT /api/projects/:id/graph` 在覆盖前把**前一版**留档到 `data/graph-backups/<projectId>.<时间戳>.json`（空图不留档，每工程保留最近 20 份，留档失败不影响正常保存）。实测：覆盖一次后备份文件内含完整的 8 节点图 —— 以后这类丢失永远可回滚，而不是靠运气找运行快照。
