...
### M1 验收（对照方案 3.3 的 15 条清单）— 全部通过（真实 Chrome + CDP 自动化）

- [x] 1 平移/缩放/触控板（25%–400%）：中键拖拽平移、滚轮缩放均实测
- [x] 2 节点拖动/多选/框选/吸附：拖拽、Shift 多选（2 节点）、Shift 框选（3 节点）、8px 网格吸附已启用
- [x] 3 Handle 连线/预览/删除：真实拖线新增边（4→5）、选中边后 Delete 删除（5→4）
- [x] 4 非法端口灰显 + 禁止光标：音频拖到 noteDoc 端口被拒绝（边数不变）；端口校验走 `isValidConnection`
- [x] 5 节点库拖入/点击添加/空白双击搜索：点击添加与双击搜索实测；拖入走 HTML5 DnD + `screenToFlowCoordinate`
- [x] 6 状态徽章渲染位就绪（running/done/error 的视觉态，M3 接真实数据）：注入状态后 `.sf-node.is-running/is-done/is-error` 均渲染
- [x] 7 产物摘要行渲染位就绪：注入 `summary` 后节点显示「12 个观点块 · 1.2k 字」
- [x] 8 右键菜单五项：实测打开，含运行×2/复制/复制输出/删除 5 项
- [x] 9 撤销/重做/复制/删除快捷键：Ctrl+D 复制、Ctrl+Z 撤销、Delete 删除均实测
- [x] 10 自动布局（分支 150–200px 错开）：elkjs 懒加载布局实测可执行，分支层级布局正常
